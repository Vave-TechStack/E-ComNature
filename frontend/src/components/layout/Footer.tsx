...
import { Store, Heart, Leaf, MessageCircle, Camera, Play, Mail, MapPin, Phone, ArrowUpRight } from 'lucide-react';
...
export function Footer() {
  return (
    <footer className="website-footer bg-noble-900 text-white">
      {/* Top Divider */}
      <div className="h-0.5 bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent" />
      ...
      <div className="container-luxury py-12 md:py-16">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-8 md:gap-12">
          ...
          <div className="col-span-2 md:col-span-1">
            ...
            <p className="text-sm text-emerald-100/50 leading-relaxed mb-6 max-w-xs">
              Pure & natural foods direct from farms, forests, and tribal communities. Authenticity guaranteed.
            </p>

            {/* Contact Info */}
            <div className="space-y-2.5 mb-6">
              <div className="flex items-center gap-2.5 text-xs text-emerald-100/40">
                <MapPin className="h-3.5 w-3.5 text-emerald-400/60" />
                <span>Hyderabad, Telangana, India</span>
              </div>
              <div className="flex items-center gap-2.5 text-xs text-emerald-100/40">
                <Mail className="h-3.5 w-3.5 text-emerald-400/60" />
                <a href="mailto:hello@naturekart.com" className="hover:text-emerald-300 transition-colors">hello@naturekart.com</a>
              </div>
              <div className="flex items-center gap-2.5 text-xs text-emerald-100/40">
                <Phone className="h-3.5 w-3.5 text-emerald-400/60" />
                <a href="tel:+918000123456" className="hover:text-emerald-300 transition-colors">+91 8000 123 456</a>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2.5">
              {[
                ...
                <a
                  ...
                  className="h-9 w-9 rounded-xl bg-white/5 hover:bg-emerald-500/20 flex items-center justify-center transition-all hover:scale-110 border border-white/5 hover:border-emerald-500/30"
                >
                  <social.icon className="h-4 w-4 text-emerald-100/60" />
                </a>
              ))}
            </div>
          </div>
              ...
              <ul className="space-y-2.5">
                {section.links.map((link) => (
                  <li key={link.label}>
                    <Link
                      href={link.href}
                      className="text-xs text-emerald-100/40 hover:text-emerald-300 transition-colors flex items-center gap-1 group/link"
                    >
                      {link.label}
        ...
        >
          <div className="flex items-center gap-4 text-xs text-emerald-100/30">
            <span className="flex items-center gap-1.5">
              <Leaf className="h-3 w-3 text-emerald-400/40" />
              100% Organic Certified
            </span>
            <span className="flex items-center gap-1.5">
              <Heart className="h-3 w-3 text-emerald-400/40" />
              Fair Trade Pledged
            </span>
          </div>
          <p className="text-xs text-emerald-100/20">
            © {new Date().getFullYear()} {APP_NAME}. All rights reserved. Made with love for nature.
          </p>
        </motion.div>
      </div>
    </footer>
