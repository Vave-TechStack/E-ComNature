...
export default function LoginForm() {
  ...
  const fillCredentials = (email: string, password: string) => {
    reset({ emailOrPhone: email, password }, { keepDefaultValues: false });
  };
    ...
    <div className="flex min-h-[80vh] items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <div className="rounded-2xl border border-primary-100 bg-gradient-to-br from-primary-50 to-accent-50 p-4 mb-6">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-sm">
              ...
              <button type="button" onClick={() => fillCredentials('admin@naturekart.in', 'Admin@123')} className="ml-auto text-[10px] text-primary-600 hover:text-primary-700 font-medium underline underline-offset-2 shrink-0">Auto-fill</button>
            </div>
            <div className="flex items-center gap-2 text-sm">
              ...
              <button type="button" onClick={() => fillCredentials('user@naturekart.in', 'User@123')} className="ml-auto text-[10px] text-accent-600 hover:text-accent-700 font-medium underline underline-offset-2 shrink-0">Auto-fill</button>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          ...
          <div>
            <label htmlFor="emailOrPhone" className="block text-sm font-medium text-gray-700 mb-1.5">Email or Phone</label>
            <div className="relative">
              <Mail className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
              <Input id="emailOrPhone" type="text" placeholder="Enter your email or phone" className="pl-10 h-12" {...register('emailOrPhone')} />
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              ...
              <Link href="/auth/forgot-password" className="text-xs font-medium text-primary-600 hover:text-primary-500 transition-colors">Forgot password?</Link>
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
              <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="Enter your password" className="pl-10 pr-10 h-12" {...register('password')} />
              <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600" aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button type="button" variant="outline" className="h-11 gap-2" onClick={() => {}}>
            ...
            <Button type="button" variant="outline" className="h-11 gap-2" onClick={() => {}}>
              <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="2" width="20" height="20" rx="4" /><path d="M7 12h10M12 7v10" /></svg> Phone OTP
            </Button>
          </div>

          <p className="mt-8 text-center text-sm text-gray-600">
            Don&apos;t have an account?{' '}
            <Link href="/auth/register" className="font-semibold text-primary-600 hover:text-primary-500 transition-colors">Create one</Link>
          </p>
        </form>
      </div>
    </div>
